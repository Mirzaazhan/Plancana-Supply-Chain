...
const BatchDetails = ({ batchId, onBack, currentUser }) => {
  ...
  const parseMLValidation = (notes) => {
    ...
    return {
      ...
      flags: flagsMatch
        ? flagsMatch[1].split(", ").map((f) => ({ type: f }))
        : [],
    };
  };
  ...
  const canUpdateStatus = () => {
    ...
    if (
      ...
      batch.farmer?.userId === currentUser.id
    ) {
      return true;
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-4 sm:py-6">
            ...
            {/* Action Buttons */}
            <div className=" items-center gap-2 min-[431px]:flex flex-wrap gap-4">
              {/* Status Badge */}
              <span
                className={`inline-flex items-center mt-2 px-3 py-1 rounded-full text-xs min-[431px]:text-sm font-medium ${getStatusColor(
                  batch.status
                )}`}
              >
              ...
              <div className="mt-2 flex flex-row items-center gap-2">
                ...
                {canUpdateStatus() && getNextStatus(batch.status) && (
                  <button
                    onClick={() =>
                      updateBatchStatus(getNextStatus(batch.status))
                    }
                    disabled={updating}
                    className="flex items-center px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 text-xs min-[431px]:text-sm"
                  >
                    {updating ? (
                ...
                <button
                  ...
                  className="flex items-center px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-xs min-[431px]:text-sm"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="border-b border-gray-200">
          <nav
            className="-mb-px grid grid-cols-3 min-[431px]:flex space-x-8"
            aria-label="Tabs"
          >
            {[
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">
                      {batch.qualityGrade || "Not specified"}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">
                    ...
                    <p className="mt-1 text-sm text-gray-900">
                      {batch.cultivationMethod || "Not specified"}
                    </p>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {batch.pricePerUnit && (
                        <div>
                          ...
                          <p className="mt-1 text-sm text-green-600 font-semibold">
                            {batch.currency || "MYR"}{" "}
                            {parseFloat(batch.pricePerUnit).toFixed(2)}/
                            {batch.unit || "kg"}
                          </p>
                        </div>
                      )}
                      {batch.totalBatchValue && (
                        <div>
                          ...
                          <p className="mt-1 text-sm text-green-600 font-semibold">
                            {batch.currency || "MYR"}{" "}
                            {parseFloat(batch.totalBatchValue).toFixed(2)}
                          </p>
                        </div>
                      )}
                      {batch.paymentMethod && (
                ...
                {batch.notes && parseMLValidation(batch.notes) && (
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    ...
                    <MLValidationBadge
                      mlValidation={parseMLValidation(batch.notes)}
                    />
                  </div>
                )}
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">
                      {batch.farmer?.user?.username || "N/A"}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">
                        {batch.farmLocation.latitude?.toFixed(6)},{" "}
                        {batch.farmLocation.longitude?.toFixed(6)}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    {
                      stage: "Processing",
                      completed: (batch.processingRecords?.length || 0) > 0,
                    },
                    {
                      stage: "Quality Test",
                      completed: (batch.qualityTests?.length || 0) > 0,
                    },
                    {
                      stage: "Transport",
                      completed: (batch.transportRoutes?.length || 0) > 0,
                    },
                    {
                      stage: "Delivered",
                      completed: ["DELIVERED", "RETAIL_READY", "SOLD"].includes(
                        batch.status
                      ),
                    },
                  ].map((stage, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              <span className="text-sm font-medium text-gray-900">
                                {transfer.fromActorName ||
                                  transfer.fromActorUsername ||
                                  "-"}
                              </span>
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-1 w-fit ${
                                  transfer.fromActorRole === "FARMER"
                                    ? "bg-green-100 text-green-800"
                                    : transfer.fromActorRole === "PROCESSOR"
                                    ? "bg-yellow-100 text-yellow-800"
                                    : transfer.fromActorRole === "DISTRIBUTOR"
                                    ? "bg-blue-100 text-blue-800"
                                    : transfer.fromActorRole === "RETAILER"
                          ...
                          <td className="px-4 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              <span className="text-sm font-medium text-gray-900">
                                {transfer.toActorName ||
                                  transfer.toActorUsername ||
                                  "-"}
                              </span>
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-1 w-fit ${
                                  transfer.toActorRole === "FARMER"
                                    ? "bg-green-100 text-green-800"
                                    : transfer.toActorRole === "PROCESSOR"
                                    ? "bg-yellow-100 text-yellow-800"
                                    : transfer.toActorRole === "DISTRIBUTOR"
                                    ? "bg-blue-100 text-blue-800"
                                    : transfer.toActorRole === "RETAILER"
                                    ? "bg-purple-100 text-purple-800"
                                    : "bg-gray-100 text-gray-800"
                                }`}
                              >
                          ...
                          <td className="px-4 py-4 whitespace-nowrap">
                            <span
                              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                transfer.transferType === "BATCH_SPLIT"
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-sm">
                            {transfer.blockchainTxId ? (
                              <span className="font-mono text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded">
                                {transfer.blockchainTxId.substring(0, 8)}...
                                {transfer.blockchainTxId.substring(
                                  transfer.blockchainTxId.length - 6
                                )}
                              </span>
                            ) : (
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700 mt-1">
                        All ownership transfers are recorded on the Hyperledger
                        Fabric blockchain, ensuring an immutable audit trail
                        (SC-03).
                      </p>
                    </div>
                  </div>
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                ...
                {(currentUser?.role === "PROCESSOR" ||
                  ...
                  <button
                    onClick={() =>
                      router.push(`/processor/quality-test/${batchId}`)
                    }
              ...
              {batch.qualityTests && batch.qualityTests.length > 0 ? (
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-medium ${
                            test.result === "PASS" || test.passed
                              ? "bg-green-100 text-green-800"
                              : test.result === "FAIL" || test.passed === false
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 max-w-md mx-auto">
                    Quality test results have not been recorded for this batch
                    yet. Tests may include pesticide residue, heavy metals,
                    microbial count, moisture content, and quality grading.
                  </p>
                </div>
              )}
              ...
              <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700 mt-1">
                      All quality tests are conducted following Malaysian
                      Agricultural Standards (MS) and recorded on the blockchain
            ...
            <div className="bg-white rounded-lg shadow p-8">
              <div className="text-center mb-8">
                ...
                <h3 className="text-2xl font-semibold text-gray-900 mb-2">
                  QR Codes for Batch {batch.batchId}
                </h3>
                <p className="text-gray-600">
                  Download and print these QR codes for verification and
                  processing
                </p>
              </div>
                ...
                <div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <ul className="text-sm text-amber-800 space-y-1 list-disc list-inside">
                          <li>
                            <strong>Verification QR:</strong> Print and attach
                            to final products for consumers to verify
                            authenticity and trace origin
                          </li>
                          <li>
                            <strong>Processing QR:</strong> Print and attach to
                            batch containers for supply chain partners to scan
                          ...
                          <li>
                            Both QR codes can be scanned with any standard QR
                            code scanner app
                          </li>
                        </ul>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-xs text-gray-600 break-all font-mono bg-white p-2 rounded border border-gray-200">
                        {qrUrls.processing ||
                          `${window.location.origin}/process-batch/${batch.batchId}`}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-center space-y-4">
                  <div className="bg-gray-100 rounded-lg p-12">
                    ...
                    <p className="text-gray-400 text-sm mt-2">
                      Click the button below to generate QR codes
                    </p>
                  </div>
                  <button
                    onClick={fetchQRCode}
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Transportation routes and logistics information (SC-06,
                    QA-07)
                  </p>
                </div>
                {(currentUser?.role === "DISTRIBUTOR" ||
                  ...
                  <button
                    onClick={() =>
                      router.push(`/distributor/transport-route/${batchId}`)
                    }
            ...
            <div className="p-6">
              {batch.transportRoutes && batch.transportRoutes.length > 0 ? (
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                        ...
                        {route.transportCost && (
                          <div className="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p className="font-semibold text-gray-900">
                              MYR {route.transportCost.toFixed(2)}
                            </p>
                          </div>
                        )}
                      ...
                      <div className="flex items-center justify-between bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-3">
                        <div className="text-center">
                          ...
                          <p className="text-xs font-medium text-gray-700">
                            {route.originLat?.toFixed(4)},{" "}
                            {route.originLng?.toFixed(4)}
                          </p>
                          {route.departureTime && (
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-xs font-medium text-gray-700">
                            {route.destinationLat?.toFixed(4)},{" "}
                            {route.destinationLng?.toFixed(4)}
                          </p>
                          {route.arrivalTime && (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 max-w-md mx-auto">
                    Transport route information has not been recorded for this
                    batch yet. Routes will appear here once the batch is in
                    transit.
                  </p>
                </div>
              )}
              ...
              <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700 mt-1">
                      All transport routes are recorded with GPS coordinates,
                      timestamps, and costs for complete supply chain visibility
                      and traceability.
                    </p>
                  </div>
                </div>
                    ...
                    >
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm text-gray-900">
                            {record.processor?.user?.username}
                          </p>
                        </div>
                      </div>
                    </div>
