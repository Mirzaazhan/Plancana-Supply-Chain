  ...
  const getSupplyChainEvents = () => {
    ...
    if (batchInfo.harvestDate) {
      events.push({
        ...
        actor:
          batchInfo.farmer?.farmName ||
          batchInfo.farmer?.user?.username ||
          "Farmer",
        location:
          batchInfo.location || batchInfo.farmLocation?.location || "Farm",
    ...
    if (supplyChain?.processingStages?.length > 0) {
      supplyChain.processingStages.forEach((stage, index) => {
        events.push({
          ...
          quantity:
            parseFloat(
              stage.outputQuantity || stage.quantity || batchInfo.quantity
            ) || 0,
          ...
          notes:
            stage.notes || `Processed by ${stage.processor || "processor"}`,
    ...
    if (distributionStages.length > 0) {
      ...
      distributionStages.forEach((stage, index) => {
        events.push({
          ...
          notes:
            stage.notes ||
            `Distributed by ${stage.distributor || "distributor"}`,
    ...
    if (
      ...
      ownershipHistory.forEach((transfer, index) => {
        ...
        events.push({
          ...
          notes:
            transfer.notes ||
            `Transferred from ${transfer.from} to ${transfer.to}`,
    ...
    if (batchInfo.status === "SOLD" || batchInfo.status === "DELIVERED") {
      ...
      const finalPrice =
        lastEvent?.price || parseFloat(batchInfo.pricePerUnit) || 0;
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-2 bg-green-50 dark:bg-green-900/30 px-3 py-1 rounded-full">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-medium text-green-700 dark:text-green-300 max-[431px]:text-xs">
                  Verified
                </span>
              </div>
            </div>
        ...
        <div className="relative w-full px-8 lg:px-16">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center space-x-4 mb-4">
              ...
              {/* ML Validation Badge */}
              {mlValidation && <MLValidationBadge validation={mlValidation} />}
            </div>
          ...
          <div className="xl:col-span-2 space-y-6">
                  ...
                  <div>
                    ...
                    {farmLocation.latitude && farmLocation.longitude && (
                      <p className="text-xs text-gray-500 font-mono">
                        {farmLocation.latitude.toFixed(4)}°N,{" "}
                        {farmLocation.longitude.toFixed(4)}°E
                      </p>
                    )}
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    setShowAgriculturalDetails(!showAgriculturalDetails)
                  }
                    ...
                    <>
                      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <p className="text-sm text-blue-800">
                          <strong>🧪 Test Mode:</strong> Displaying sample data
                          with quantity tracking, price changes, and batch
                          splits for demonstration purposes.
                        </p>
                        <div className="mt-2 flex flex-wrap gap-2">
                          <button
                            onClick={() => {
                              /* Could cycle through test scenarios */
                            }}
                      ...
                      <SupplyChainFlowchart
                        batchData={testScenarios.withSplits}
                      />
                    </>
                  ...
                  {supplyChainEvents.length > 0 ? (
                    supplyChainEvents.map((event, index) => (
                      <div key={index} className="flex items-start space-x-4">
                        <div
                          className={`w-12 h-12 bg-${event.color}-100 rounded-full flex items-center justify-center flex-shrink-0`}
                        >
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <p className="text-xs text-blue-800">
                      <strong>💰 Price Transparency:</strong> Farm-gate prices
                      are recorded on blockchain to ensure fair pricing
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">
                    {formatDate(
                      traceability.harvestDate || batchInfo.harvestDate
                    )}
                  </span>
                </div>
            ...
            <div className="bg-green-50 rounded-lg p-6">
              ...
              <div className="space-y-2">
                <p className="text-sm text-green-800">
                  ...
                  {formatDateTime(verificationData?.verificationTime)}
                </p>
                <p className="text-sm text-green-800">
                  <strong>Blockchain Hash:</strong>
                  <span className="font-mono text-xs break-all block mt-1">
                    {verificationData?.verification?.dataIntegrity?.databaseHash?.substring(
                      0,
                      32
                    )}
                    ...
                ...
                <p className="text-sm text-green-800">
                  ...
                  {verificationData?.verification?.dataIntegrity?.message ||
                    "Verified"}
                </p>
                <div className="mt-4 p-3 bg-white rounded border border-green-200">
                  <div className="flex items-center text-sm text-green-700">
                    ...
                    This product is authentic and verified through blockchain
                    technology
                  </div>
                </div>
              </div>
        ...
        <div className="w-full px-6 lg:px-12 xl:px-16 py-6">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center space-x-4 text-xs text-gray-500 dark:text-gray-500">
              ...
              <span>•</span>
              <span>
                Verified: {formatDateTime(verificationData?.verificationTime)}
              </span>
              <span>•</span>
              <span>
                Blockchain:{" "}
                {verificationData?.verification?.blockchain
                  ? "Active"
                  : "Verified"}
              </span>
            </div>
