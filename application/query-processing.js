...
async function queryProcessingRecords(batchId) {
    ...
    try {
        ...
        if (!identity) {
            throw new Error('Identity "appUser" not found in wallet. Please enroll the user first.');
        }
        ...
        if (processingRecords.length === 0) {
            ...
            console.log('═══════════════════════════════════════════════════════════════');
            console.log(`🏭 PROCESSING RECORDS (${processingRecords.length} record${processingRecords.length > 1 ? 's' : ''})`);
            console.log('═══════════════════════════════════════════════════════════════\n');
            ...
            processingRecords.forEach((record, index) => {
                ...
                console.log(`  Location:           ${record.processingLocation || 'Not specified'}`);
                console.log(`  Coordinates:        ${record.latitude || 'N/A'}, ${record.longitude || 'N/A'}`);

                if (record.latitude && record.longitude) {
                    console.log(`  Maps Link:          https://www.google.com/maps?q=${record.latitude},${record.longitude}`);
                }

                console.log(`\n  📊 Quantities:`);
                console.log(`    Input:            ${record.inputQuantity || 'N/A'} ${batch.unit || 'kg'}`);
                console.log(`    Output:           ${record.outputQuantity || 'N/A'} ${batch.unit || 'kg'}`);
                console.log(`    Waste:            ${record.wasteQuantity || 'N/A'} ${batch.unit || 'kg'}`);
                ...
                console.log(`\n  ⚡ Resources:`);
                console.log(`    Processing Time:  ${record.processingTime || 'N/A'} minutes`);
                console.log(`    Energy Usage:     ${record.energyUsage || 'N/A'} kWh`);
                console.log(`    Water Usage:      ${record.waterUsage || 'N/A'} liters`);
        ...
        if (batch.transportRecords && batch.transportRecords.length > 0) {
            console.log(`🚚 Transport Records: ${batch.transportRecords.length} records`);
        }
        ...
        if (batch.financialTransactions && batch.financialTransactions.length > 0) {
            console.log(`💰 Financial Transactions: ${batch.financialTransactions.length} transactions`);
        }
