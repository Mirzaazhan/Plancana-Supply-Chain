...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const networkResult = await blockchainService.connectToNetwork();
      let blockchainBatches = [];

      if (networkResult && networkResult.gateway && networkResult.contract) {
        const { gateway, contract } = networkResult;
        try {
