...
class BlockchainQuery {
    ...
    async getAllBatches(filters = {}) {
        const result = await this.contract.evaluateTransaction('getAllBatches', JSON.stringify(filters));
        return JSON.parse(result.toString());
    }
    ...
    async getPricingHistory(batchId) {
        const result = await this.contract.evaluateTransaction('getPricingHistory', batchId);
        return JSON.parse(result.toString());
    }

    async getTransportHistory(batchId) {
        const result = await this.contract.evaluateTransaction('getTransportHistory', batchId);
        return JSON.parse(result.toString());
    }
}
...
function displayProcessingRecords(records, unit = 'kg', format = 'table') {
    ...
    if (!records || records.length === 0) {
        console.log(`${colors.yellow}ℹ️  No processing records found${colors.reset}\n`);
        return;
    }
    ...
    records.forEach((record, index) => {
        ...
        if (record.latitude && record.longitude) {
            ...
            console.log(`Google Maps:      https://maps.google.com/?q=${record.latitude},${record.longitude}`);
        }
        ...
        if (record.inputQuantity && record.outputQuantity) {
            const efficiency = ((record.outputQuantity / record.inputQuantity) * 100).toFixed(2);
            console.log(`  Efficiency:     ${colors.green}${efficiency}%${colors.reset}`);
        }
...
async function handleGetBatch(batchId, options) {
    ...
    try {
        ...
        if (options.json) {
            ...
            console.log(`Crop:             ${batch.crop} ${batch.variety ? `(${batch.variety})` : ''}`);
            ...
            if (batch.coordinates) {
                console.log(`Coordinates:      ${batch.coordinates.latitude}, ${batch.coordinates.longitude}`);
            }
            ...
            } else if (batch.processingRecords && batch.processingRecords.length > 0) {
...
async function handleGetProcessing(batchId, options) {
    ...
    try {
        ...
        displayProcessingRecords(batch.processingRecords, batch.unit, options.json ? 'json' : 'table');
    ...
    try {
        ...
        if (options.json) {
            ...
            batches.forEach((batch, index) => {
                console.log(`${index + 1}. ${colors.bright}${batch.batchId}${colors.reset}`);
                console.log(`   Farmer: ${batch.farmer}`);
                console.log(`   Crop: ${batch.crop} | Qty: ${batch.quantity} ${batch.unit || 'kg'}`);
                console.log(`   Status: ${colors.green}${batch.status}${colors.reset}`);
                console.log(`   Processing Records: ${batch.processingRecords?.length || 0}`);
                console.log('');
            });
        }
...
async function handleHistory(batchId, options) {
    ...
    try {
        ...
        if (options.json) {
            ...
            console.log('═'.repeat(70));
            console.log(`Current Status:     ${colors.green}${history.currentStatus}${colors.reset}`);
...
`);
    ...
    const options = {
        ...
        farmer: args.includes('--farmer') ? args[args.indexOf('--farmer') + 1] : null,
    ...
    try {
        switch (command) {
            case 'batch':
                if (!batchId) {
                    console.error(`${colors.red}❌ Error: Batch ID required${colors.reset}\n`);
                    process.exit(1);
                }
